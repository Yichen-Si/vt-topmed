...
VNTRAnnotator::VNTRAnnotator(std::string& ref_fasta_file, bool _debug, uint32_t _m, int32_t _c) : debug(_debug), max_mlen(_m), min_copy(_c) {
...
void VNTRAnnotator::find_repeat_unit(bcf_hdr_t* h, bcf1_t* v, std::set<candidate_unit>& candidate_ru) {
    ...
    if (debug) {
        std::cerr << "============================================\n";
        std::cerr << "Read indel: " << v->pos << '\t' << alleles[0] << '\t' << alleles[1] << '\t';
        char* seq = faidx_fetch_seq(fai, chrom, v->pos-20, v->pos+20, &seq_len);
        printf("%s\n", seq);
        free(seq);
    }
    ...
    if (extended_seq.length() > 5) {
        if (debug) {
            std::cerr << "Query after right align " << extended_seq << '\n';
        }
    ...
    if (extended_seq.length() > 5) {
        if (debug) {
            std::cerr << "Query after left align " << extended_seq << '\n';
        }
        ct_indel += rl_find_repeat_unit(extended_seq, candidate_ru, 0);
    }
    ...
    if (extended_ed0 - indel_end  + 1 < pad) {
        ...
        char* seq = faidx_fetch_seq(fai, chrom, v->pos+1, extended_ed0, &seq_len);
        extended_seq.assign(seq);
        free(seq);
    ...
    if (v->pos - extended_st0 < pad) {
        ...
        char* seq = faidx_fetch_seq(fai, chrom, extended_st0, v->pos, &seq_len);
        extended_seq.assign(seq);
    ...
    if (ct_indel == ct0) {
        pad = std::min(pad, 12);
        char* seq = faidx_fetch_seq(fai, chrom, v->pos-pad, v->pos+pad, &seq_len);
        extended_seq.assign(seq);
        free(seq);
...
void VNTRAnnotator::find_repeat_region(bcf_hdr_t* h, bcf1_t* v, std::set<candidate_unit>& candidate_ru, std::vector<candidate_fuzzy_motif>& candidate_model) {
    ...
    char* seq = faidx_fetch_seq(fai, chrom, start0, v->pos, &seq_len);
    std::string query(seq);
    free(seq);
    // std::string query = faidx_fetch_seq(fai, chrom, start0, v->pos, &seq_len);
    start0 = v->pos - seq_len + 1;
    ...
    seq = faidx_fetch_seq(fai, chrom, indel_end+1, end0, &seq_len);
    query.append(seq);
    free(seq);
    ...
    for (const auto& s : candidate_ru) {
        ...
        //     }
            ...
            if (is_insertion) {
                ...
                if (real_st > real_ed) { // because repeat region is entirely inside insertion
                    real_st = real_ed;
                }
            }
            candidate_fuzzy_motif model(display_ru, inexact_label, real_st, real_ed, rr.nr, rr.score, l, rr.match_motif);
            if (is_insertion && rr.p_ed >= spiked_st && rr.p_st <= spiked_ed) { // Decide if the inserted sequence is relevant (need to be explained by final model) to the TR.
                if (wphmm->ru_complete[spiked_ed] - wphmm->ru_complete[spiked_st-1] > 0) {
                    model.insertion_relevant = 1;
                } else {
                    std::set<char> s1;
                    int32_t nn = 0;
            ...
            if (debug) {
                ...
                std::cerr << '\t' << model.viterbi_score << '\t' << model.concordance << '\t' << rr.match_motif << '\t' << rr.p_ed - rr.p_st + 1 << '\t' << rr.nr << '\t' << model.insertion_relevant << '\t' << real_st << '-' << real_ed << '\t' << query.length() << '\n';
...
int32_t VNTRAnnotator::rl_find_repeat_unit(std::string& context, std::set<candidate_unit>& candidate_ru, bool flag) {
    ...
    if (cseq.size() > 2) {
        ...
        for (auto & s: pseq_obj.candidate) {
            candidate_unit tmp(s,1);
            if (substr_to_candidate_unit(s, cseq, rl, tmp)) {
                candidate_ru.insert(tmp);
                added_ct++;
            }
        }
...
int32_t VNTRAnnotator::if_homopoly(const char* chrom, int32_t left, int32_t right, char&b) {
    int32_t seq_len;
    char* seq = faidx_fetch_seq(fai, chrom, std::max(0, left-2*HOMOPOLYMER_MIN), left, &seq_len);
    std::string hseq(seq);
    free(seq);

    // std::string hseq = faidx_fetch_seq(fai, chrom, std::max(0, left-2*HOMOPOLYMER_MIN), left, &seq_len);
    uint32_t st_rel = hseq.size();
    seq = faidx_fetch_seq(fai, chrom, right, right+2*HOMOPOLYMER_MIN, &seq_len);
    hseq.append(seq);
    free(seq);
...
bool VNTRAnnotator::choose_models(VNTR_candidate& vntr, int32_t mode) {
        ...
        std::set<candidate_unit> candidate_ru;
        std::string query = vntr.query.substr(vntr.rel_st, vntr.len_mrg);
        std::string cseq;
        std::vector<int32_t> rl;
        to_rl(query, cseq, rl);

        // Simplify candidate fuzzy models?
        auto it = vntr.alternative_models.begin();
        while (it != vntr.alternative_models.end()) {
            candidate_fuzzy_motif& v = *it;
            candidate_unit tmp(v.ru_org, 1);
            if (substr_to_candidate_unit(v.ru_org, cseq, rl, tmp) ) {
                if (!tmp.inexact && !v.inexact) {
                    it++;
                    continue;
                }
                bool updated = (v.inexact != tmp.inexact);
                for (uint32_t i = 0; i < v.mlen; ++i) {
                    bool fz = tmp.variable_base[i].first < tmp.variable_base[i].second;
                    if (v.label[(i+v.mlen-v.phase)%vntr.mlen] != fz) {
                        updated = 1;
                        break;
                    }
                }
        ...
        // Double check if there is better candidate repeat unit missed
        int32_t ct = rl_find_repeat_unit(query, candidate_ru);
        ct = 0;
        for (auto & s : candidate_ru) {
            if (s.ru.length() == 1) {
                continue;
            }
            std::vector<bool> inexact_label(s.ru.length(), 0);
            if (s.inexact) {
                for (size_t i = 0; i < s.ru.size(); ++i) {
                    if (s.variable_base[i].first < s.variable_base[i].second) {
                        inexact_label[i] = 1;
                    }
                }
            }
            candidate_fuzzy_motif mot(s.ru, inexact_label);
            bool novel = 1;
            for (auto & v : vntr.alternative_models) {
                if (v.ru_compare(mot) == 0) {
                    novel = 0;
                    break;
                }
            }
            if (novel) {
                vntr.alternative_models.push_back(mot);
                ct++;
            }
        }
        ...
        it = vntr.alternative_models.begin();
        bool ex = 0, fz = 0;
        while (it != vntr.alternative_models.end()) {
            if (fz && ex) {
                it = vntr.alternative_models.erase(it);
        ...
        int32_t n_model = 0;
        for (auto & mot : vntr.alternative_models) {
            if (mot.mlen > 1) {
                n_model++;
                continue;
            }
            int32_t p_st = vntr.repeat_ref.length()/2;
            int32_t p_ed, nr = 0;
            find_homopoly_region(vntr.repeat_ref, mot.ru.at(0), p_st, p_ed, nr);
            if (n_model == 0) {
                vntr.score_r       = nr*2;
                vntr.n_ru_r        = nr;
                vntr.rlen_r        = p_ed - p_st + 1;
                vntr.concordance_r = (double) nr / vntr.rlen_r;
            } else {
                vntr.score_r2       = nr*2;
                vntr.n_ru_r2        = nr;
                vntr.rlen_r2        = p_ed - p_st + 1;
                vntr.concordance_r2 = (double) nr / vntr.rlen_r2;
            }
            n_model++;
        }
        return 1;
 ...
 */
 bool VNTRAnnotator::is_vntr(candidate_fuzzy_motif& motif, int32_t mode)
 {
     ...
     if (mode==VITERBI_VNTR) {
         ...
         if (mlen==1) {
             return (motif.concordance > 0.85 && motif.n_ru > 6);
         }
         if (motif.n_ru < min_copy || motif.concordance < 0.75 || rlen - mlen < 6) {
             return false;
         }
         if (motif.inexact) {
             if (motif.n_ru < 3) {
  ...
  * Helpers
  */
 void VNTRAnnotator::to_rl(std::string& seq, std::string& cseq, std::vector<int32_t>& rl) {
     if (seq.length() == 0) {
         return;
     }
     cseq = seq.at(0);
     rl.clear();
     char pre_base = seq.at(0);
     ...
     for (uint32_t i = 1; i < seq.size(); ++i) {
         if (seq.at(i) != pre_base) {
             if (base_ct.find(seq.at(i))==base_ct.end()) {
                 continue;
             }
             cseq += seq.at(i);
             base_ct[seq.at(i)] += 1;
...
bool VNTRAnnotator::substr_to_candidate_unit(std::string& s, std::string& cseq, std::vector<int32_t>& rl, candidate_unit& tmp) {
